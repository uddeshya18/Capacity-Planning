...
if merc_file and qc_file:
    ...
    # 2. DEDUPLICATION (Batch Level) - Essential for en_US accuracy
    ...
    # 3. IDENTIFY RECENTLY ACTIVE TASKS (To hide ghost tasks)
    ...
    # 4. FILTERING & GROWTH (Using FULL history for stable growth)
    ...
    # Calculate Growth using full dataset for accuracy
    site_growth_val = 0.0
    if not f_q.empty:
    ...
    # 5. AGGREGATION (Baseline = Avg of last 4 weeks only)
    num_weeks = 4
    ...
    with tab1:
        ...
        with col1:
            ...
            for _, row in wf_base.iterrows():
                # HIDE IF NOT ACTIVE IN LAST 2 WEEKS
    ...
    st.info("Upload files to see current active tasks with stable growth trends.")
